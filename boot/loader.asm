...
LABEL_START:
	...
	;get the num of memory
	mov ebx,0
	mov di,_MemChkBuf 	;es:di point to a Address Range Descriptor Structure
						;ARDS
.MemChkLoop:
	mov eax,0E820h
	mov ecx,20
...
InitKernel:
	xor esi,esi
	mov cx,word [BaseOfKernelFilePhyAddr+2Ch];ecx=pELFHdr->e_phnum,program header num
	movzx ecx,cx
	mov esi,[BaseOfKernelFilePhyAddr+1Ch];esi=pELFHdr->phoff
	add esi,BaseOfKernelFilePhyAddr ;esi+=OffsetOfKernel
.Begin:
	mov eax,[esi+0]
	cmp eax,0
	jz .NoAction
	push dword [esi+010h]	;size
	mov eax,[esi+04h]		;memcpy((void*)(pPHdr->p_vaddr),
	add eax,BaseOfKernelFilePhyAddr;BaseOfLoaderPhyAddr+p_offset
	push eax 						;p_filesz)
...
szPMMessage 	equ BaseOfLoaderPhyAddr + _szPMMessage
